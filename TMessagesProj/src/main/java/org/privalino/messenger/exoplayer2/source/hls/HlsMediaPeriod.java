  ...
  private void buildAndPrepareSampleStreamWrappers(long positionUs) {
    ...
    for (int i = 0; i < audioRenditions.size(); i++) {
      sampleStreamWrapper = buildSampleStreamWrapper(C.TRACK_TYPE_AUDIO,
          new HlsMasterPlaylist.HlsUrl[] {audioRenditions.get(i)}, null, Collections.<Format>emptyList(), positionUs);
      sampleStreamWrappers[currentWrapperIndex++] = sampleStreamWrapper;
      sampleStreamWrapper.continuePreparing();
    }
    ...
    for (int i = 0; i < subtitleRenditions.size(); i++) {
      ...
      sampleStreamWrapper = buildSampleStreamWrapper(C.TRACK_TYPE_TEXT, new HlsMasterPlaylist.HlsUrl[] {url}, null,
